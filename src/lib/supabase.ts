...
// PostgREST caps every response at 1000 rows, so any unpaged select silently
// truncates once a table grows past that (eventi_prog already has). Page
// through the query instead of trusting a single response.

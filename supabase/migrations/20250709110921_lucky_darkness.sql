/*
  ...
  1. Ensure admin user has correct profile with user_role = 'admin'
  2. Replace all eventi_prog RLS policies with hardcoded admin UUID
  3. Remove dependency on is_admin() function in RLS policies
  4. Grant necessary permissions for admin access
*/
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'WARNING: Admin user alboabourt@progdealer.com not found in auth.users!';
    RAISE NOTICE 'Creating fallback policies without admin access';
    
    -- Create fallback policies without admin access
    DROP POLICY IF EXISTS "Public can read approved events" ON eventi_prog;
    DROP POLICY IF EXISTS "Public can submit manual events" ON eventi_prog;
